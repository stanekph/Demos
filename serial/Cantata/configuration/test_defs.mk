...
$(THIS_TEST)/decode_results:
	$(DEMULT_CMD)
#
# Rule to run the test if it failed during the last run.
#
.PHONY : $(THIS_TEST)/run_if_failed
$(THIS_TEST)/run_if_failed:
	@$(CHECK_STATUS_CMD);\
	if [ $$? -ne 0 ]; \
    then \
       $(ECHO_CMD_MED) Running $(DYNAMIC_THIS_TEST) ...;\
       $(MAKE) $($(DYNAMIC_THIS_TEST)_BUILD_AND_RUN) --warn-undefined-variables -s -k -f $(DYNAMIC_THIS_TEST)/test.mk;\
    fi
#
# Clean rule for re-run the test which failed last run.
#
.PHONY : $(THIS_TEST)/clean_except_results
$(THIS_TEST)/clean_except_results :
	$(CLEAN_EXCEPT_RESULT_CMD)
#
# Clean rule
#
